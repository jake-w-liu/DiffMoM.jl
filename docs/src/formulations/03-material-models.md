...
- **`test/test_material_models3d.jl`** -- the dedicated `"3D material model helpers"` testset. It covers static and dispersive electric and magnetic responses, bianisotropic tensors, passivity enforcement and opt-out, invalid shapes and frequencies, exceptional numeric ranges, and `MagneticMaterial3D` delegation through both stored models.
