...
| 2026-08-26 | INV-044, INV-048 | The reviewed quality contract runs the same bounds-checked root project test command as the passing platform jobs; clean temporary resolution remains a separate required gate, and ignored root/docs manifests are not treated as protected source. | CI platform tests passed on Ubuntu, macOS, and Windows; after the command was aligned, the gate passed its suite but flagged creation of the intentionally ignored root `Manifest.toml`, just as the docs gate had flagged its ignored manifest. |
...
| 2026-10-12 | INV-012, INV-048 | Near-field matrices assemble directly in column-major CSC form; the triplet intermediate, its `sparse()` consolidation, and the `max_triplet_bytes` bound on that transient are removed (`solve_scattering` no longer accepts the keyword; registered drivers no longer set it). | The triplet layer held about 1 GiB transient for the level-2 case needing 16,779,488 entries; direct CSC assembly keeps only the ~0.4 GiB final payload. The user directed real allocation efficiency instead of a transient bound. |
